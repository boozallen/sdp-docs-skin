...
function buildSiteUiModel (playbook, contentCatalog, navigationCatalog) {
  ...
  */
  for(let component in site.components){
    // build navigation data for latest version 
    site.components[component].latestVersion.navigation = navigationCatalog.getNavigation(component.name, site.components[component].latestVersion.version) || []
    // build navigation data for each version in array 
    for(let versionIndex in site.components[component].versions){
      let v = site.components[component].versions[versionIndex]
      site.components[component].versions[versionIndex].navigation = navigationCatalog.getNavigation(component.name, v.version) || []
    }
  }
